...
int main(void){
    ...
    while (1){
		...
		for(unsigned int x = (int)(min_duty_cyc*OCR0A); x <= (int)(max_duty_cyc*OCR0A); x += (int)(duty_per_deg*delta_theta)){
			//for(unsigned int x = 5; x <= 16; x ++){
			OCR0B = x;
			
			 //motor angle type = unsigned short
			...
			//^^    important for passing information to the visual processing. 
			
			if(object_distance > 400){
				
				
				
				//printf("Object out of range. Angle = %u.\n", motor_angle); //				<----- uncomment this later
				
				
				printf("Object is out of range. Angle = %u. OCR0B = %u. \n", motor_angle, OCR0B); //<------ remove this later
				OCR2A = audio_feedback(object_distance);
				
			}
			else{
				printf("Object distance = %u cm. Angle = %u. OCR0B = %u. \n", object_distance, motor_angle, OCR0B);
				OCR2A = audio_feedback(object_distance);

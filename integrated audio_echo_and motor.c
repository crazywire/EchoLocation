/*
 * integrated audio, speaker, motor 
 *
 * Created: 3/13/2018 
...
const float min_pos_t = 0.55e-3; //1ms
    ...
    while (1){
		...
		for(unsigned int x = (int)(min_duty_cyc*OCR0A); x <= (int)(max_duty_cyc*OCR0A); x += (int)(duty_per_deg*delta_theta)){
			...
			if(object_distance > 400){
				...
				PORTD |= (1<<PD7); //enable the pwm signal to the speaker
				_delay_ms(500);
				PORTD &= ~(1<<PD7); //disable the pwm signal to the speaker

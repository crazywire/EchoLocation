...
void configure_timers(){
	...
	TCNT1=0;
	OCR1A = (int)(fclk*trigger_pulse_length/64); 
	//Top value for compare ~10us
}
...
void configure_external_interrupts(){
	...
	//enable input capture interrupt & output compare interrupt
	sei();
}
...
int main(void){
	...
	while(1){
		if(echo_flag){
			echo_flag = 0;
			PORTB |= (1<<PB1); 
			// start trigger pulse(ISR turns this pin off);
